...
import { formatTime, formatDuration, eventDuration, CATEGORY_COLORS, type LogEntry } from '../lib/dbus'
...
<template>
  <div class="timeline">
    ...
    <div v-for="log in logs" :key="'r' + log.id" class="entry" @click="selected = log" :class="{ on: selected?.id === log.id }">
      ...
      <span class="badge" :style="{ background: CATEGORY_COLORS[log.category] ?? '#555' }">{{ log.category }}</span>
      <span class="app">{{ log.friendly_name }}</span>
